"""
StatScout MCP Server
Custom MCP server exposing 3 tools for OSF dataset discovery and analysis.
Transport: stdio (runs as subprocess from the ADK agent)
"""
...
async def _search_osf_projects(query: str, limit: int) -> dict:
    """Search OSF nodes (projects) by title/description keyword."""
    ...
    # If title filter returned nothing, fall back to a broader search via OSF search API
    if not projects:
...
async def _get_dataset_files(project_id: str) -> dict:
    """List CSV files available in an OSF project's storage."""
...
async def _fetch_csv_preview(file_url: str, rows: int) -> dict:
    """Download CSV, compute descriptive stats, return structured preview."""
    ...
    categorical_summary = {}
    for col in cat_df.columns[:5]:  # cap at 5 categorical columns
        vc = df[col].value_counts().head(5).to_dict()
        categorical_summary[col] = {str(k): int(v) for k, v in vc.items()}

    # Head preview
    head_records = df.head(rows).to_dict(orient="records")
    # Truncate string values to avoid bloating the response
    cleaned_head = []
